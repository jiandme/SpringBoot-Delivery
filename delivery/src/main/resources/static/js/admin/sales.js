...
function graphDraw(data, format, title){
	...
	for(i=0;i<data.length-1;i++) {
		...
		`<li>
			...
			<div class="graph" data-date="${moment(data[i].orderDate).format("YYYY-MM-DD")}"></div>
...
function sales(term){
	...
	$.ajax({
		url: "/api/admin/management/sales",
		type: "GET",
		data: data
	})
...
function salesDetail(data, fnc){
	console.log(data);
	$.ajax({
		url: "/api/admin/management/salesDetail",
		type: "GET",
		data: data
	})
	.done(function(result){
...
$(".other_detail .sort_name").click(function(){
	const date =$("#other_detail_date").text();
	console.log("얏호" + date);
	const data ={
		storeId : storeId,
		date : date
