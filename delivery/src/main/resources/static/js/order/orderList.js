...
$(document).ready(function() {
	...
	$(".review").click(function() {
		...
		if ($(this).hasClass("regi")) {
			...
			if(reviewImg !=""){
				...
				$(".review_modify_modal .img_box div").css("display", "none");
			}
		}
	...
	$(".order_detail").click(function() {
		const orderNum = $(this).siblings(".order_num").val();
		location.href = "/orderDetail/" + orderNum;
	});
...
function reviewUpload(event){
	...
	formData.append("storeId",$('[name=storeId').val())
	formData.append("orderNum",$('[name=orderNum').val())
	formData.append("reviewContent",$('[name=reviewContent').val())
	formData.append("score",$('[name=score').val())
